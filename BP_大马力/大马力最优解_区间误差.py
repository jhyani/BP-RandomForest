...
#         print("absolute difference: ",accp1)
        ...
        gap_abs = np.abs(test_predict-test_y[:len(test_predict)])
#         print ("预测值  ","  真实值")
        for i in range(len(test_predict)):
            ...
            gap.append(gap_abs[i])
#             print("%s: "%i,test_predict[i],test_y[i], acc[i])
        
        return true,pre,error, error_abs, gap
def split_range(df):
    t = []
    o_abs = []
    tr_sum = []
    ...
    for i in range(iter):
        if i <7 :
            t.append(df[(df['马力'] >=100+20*i) & (df['马力'] <100+20*(i+1))])
            engin.append("%s-%s"%(100+20*i,100+20*(i+1)))
        else:
            if len(df[df['马力'] >= 240]) >= 1:
                t.append(df[(df['马力']>= 240) ])
                engin.append(">240")

        o1 = t[i]['实际值'].sum()
        o2 = t[i]['pre'].sum()
#         o_abs.append(abs(o1-o2)/o1)
        o_abs.append((o2-o1)/o1)
        o.append((o1-o2)/o1) 
        tr_sum.append(o1)
        pre_sum.append(o2)
...
#     engin = ['100-120','120-140','140-160','160-180','180-200','200-220','220-240', '>240']
    for n, o_abs in enumerate([o_abs1, o_abs2]):
        for i, j in zip(engin[n],o_abs):
...
#     plt.ylim([-10, 10])
    ...
    plt.legend()
    plt.show()
    for i,j,k in zip(engin1,tr_sum1,pre_sum1):
        print("区间   实际值和    预测值和")
        print("%s: %s   %s"%(i,j,k))
...
def main1():
    ...
    for i,j in enumerate(all):
        for k in range(5):
            ...
            true,pre,error, error_abs, gap= prediction(pred,x,1,data_test[i])
#             print(true, pre)
            data_out[i] = data_test_ori[i]
            data_out[i]['true'] = true
            data_out[i]['pre_%s'%k] = pre
            data_out[i]['gap_%s'%k] = gap
    #         data_out[i]['error'] = error
            data_out[i]['error_abs_%s'%k] = error_abs
#             stderr[k] = data_out[i]['error_abs_%s'%k].abs().mean()   ## 按照绝对值
            stderr[k] = data_out[i]['gap_%s'%k].abs().mean()      ###按照绝对值误差来选最优次
...
if __name__ == '__main__':
    os.remove('大马力最优解结算结果-1106.txt')
    log = open('大马力最优解结算结果-1106.txt', 'a+')    
    main1()
