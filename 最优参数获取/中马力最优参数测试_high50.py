...
data1_1 = data1[data1['实际值'] <= 50]
data1_2 = data1[data1['实际值'] > 50]
...
for i in range(len(data2)):
    if data2['实际值'][i] < 1:
        data2['实际值'][i] = 1
data2_1 = data2[data2['实际值'] <= 50]
data2_2 = data2[data2['实际值'] > 50]
data2_3 = data2[data2['实际值'] <= 50]
data2_4 = data2[data2['实际值'] > 50]
...
def run_model(loss_function, optimizer, pred, x_data, y_data, x, y, b, w,train_epochs):
    ...
    module_file = tf.train.latest_checkpoint('./model-4')
...
#             print("epoch=", epoch + 1, "loss=", loss_average, "b=", b0temp, "w=", w0temp)
            ...
            if epoch % 200==0:
                print("保存模型：",saver.save(sess,'./model-4/stock2.model',global_step=epoch))
    ...
    for m in lr:
        for n in echo:
            ...
            for k in range(5):
                ...
                data_out[repeat] = data_test_ori[1]
...
#                 rec.flush()
    ...
    pd.set_option('display.width', 120) # 设置打印宽度(**重要**)

    
    writer = pd.ExcelWriter('output_new_high_50_test.xls',engine='xlsxwriter')
    start_row = 1
    for i in range(25):
...
if __name__ == '__main__':
    if os.path.exists('./error_record_high_50.txt'):
        os.remove('error_record_high_50.txt')

    log = open('error_record_high_50.txt', 'a+')
    main1()
